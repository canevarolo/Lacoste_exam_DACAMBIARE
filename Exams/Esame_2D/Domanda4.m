...
vettore=[ 2.5e-3 5e-3 1e-2 2.5e-2 5e-2];
for gg=1:length(vettore)
    ...
    if gg==1
        ...
        err(gg)=norm(Tmax-Tref(1:round(dy/dyref):nref(3)))/norm(Tref(1:round(dy/dyref):nref(3))-Ta);
    end

end
toll=1e-3*ones(length(vettore),1);
loglog(vettore,err,'r-o','LineWidth',2)
hold on
loglog(vettore,toll,'k-o','LineWidth',2)
grid on
title('Convergenza')